...
/// <summary>
/// The WhatsNot pricer's fallback when stored history comes up empty: one budget-gated live
/// OpenWebNinja lookup, then the same ask again. Before this, a niche lot on a live show read
...
public class WhatsNotLiveCompsFallbackAssetTests
{
    ...
    public void A_no_data_card_asks_eBay_live_and_then_asks_again()
    {
        ...
        Assert.Contains("await runLiveLookup(q, 'wn')", Js);
    ...
    {
        // A second no_data after a fetch that FOUND rows means the sold titles don't match the
        // lot's name — more fetching cannot fix that, and a loop would spend the day's budget
        // proving it. The guard is a module flag, NOT a parameter: wnPriceItem is wired straight
        // into click handlers, and a parameter would arrive holding the click Event — truthy —
        // so the fallback would never fire from the button at all.
    ...
    public void A_lookup_that_could_not_help_says_why_in_its_own_words()
    {
        ...
        Assert.Contains("const why = run.message || 'Live lookup found nothing new.';", Js);
    }
